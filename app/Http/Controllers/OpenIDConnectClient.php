 ...
 *
 */
class OpenIDConnectClient extends Controller
{
    ...
    private $scopes = ['api1','openid','profile'];
     ...
     */
    private $codeChallengeMethod = true; // I changed this mm2604
     ...
     */
    protected function addAdditionalJwk($jwk) { //Not using
    ...
    {   
        ...
        //return sprintf('%s://%s%s/%s', $protocol, $host, '/', trim(reset($explodedRequestUri), '/'));
        return sprintf('%s://%s%s/%s', $protocol, $host, '', trim(reset($explodedRequestUri), '/'));
    ...
    private function requestAuthorization() {
        ...
        $auth_params = array_merge($this->authParams, [
            ...
            //'redirect_uri' => 'https://dev2-portal.delivercarerx.com/securedlccontrol/openId',
    ...
    protected function fetchURL(string $url, string $post_body = null, array $headers = []) {
        ...
        if ($output === false) {
            ...
            // We want them to see 404 error
            throw new OpenIDConnectClientException('Curl error: (' . curl_errno($ch) . abort('404') . ') ' . curl_error($ch));
            $data = ['MESSAGE' => curl_errno($ch) ?? "", 'EMAIL' => 'mmartinez@delivercarerx.com'];
            EmailTemplatesRepository::sendMail('OpenIdConnect: Line 1461', $data);
     ...
     */
    public function setPrivateKeyJwtGenerator(callable $privateKeyJwtGenerator) { // Not using
        $this->privateKeyJwtGenerator = $privateKeyJwtGenerator;
    }

    public function setAllowImplicitFlow(bool $allowImplicitFlow) { // Not using
     ...
     *        simple key => value
     */
    public function providerConfigParam(array $array) { // Not using
    ...
    public function setClientName(string $clientName) { //Not using
     ...
     */
    public function setAccessToken(string $accessToken) { //Not using
        $this->accessToken = $accessToken;
    }
     ...
     * @return object
     */
    public function getAccessTokenHeader() { //Not using
     ...
     * @return object
     */
    public function getAccessTokenPayload() { //Not using
     ...
     * @return object
     */
    public function getIdTokenPayload() { //Not using
     ...
     * @return object
     */
    public function getTokenResponse() { //Not using
        return $this->tokenResponse;
    }
